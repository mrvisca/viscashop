...
@section('content')
    ...
    <div class="content">
        ...
        <div class="top-bar">
            <!-- BEGIN: Breadcrumb -->
            <div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="{{ route('admin.product') }}" class=""> Produk</a> <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Edit Produk</a> </div>
            <!-- END: Breadcrumb -->
            <!-- BEGIN: Notifications -->
            ...
            <div class="px-5 sm:px-20 mt-5 pt-5">
                <div class="grid grid-cols-12 gap-4 row-gap-5 mt-5">
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        ...
                        <input type="text" class="input w-full border flex-1 ini-stock" placeholder="Job, Work, Documentation" disabled>
                    </div>
                    <div class="intro-y col-span-12 sm:col-span-6">
                        <div class="mb-2">Satuan</div>
                        <select class="input w-full border flex-1 ini-satuan" disabled>
                    ...
                    <div class="intro-y col-span-12 flex items-center justify-center sm:justify-end mt-5">
                        ...
                        <button class="button w-24 justify-center block bg-theme-1 text-white ml-2 update-produk">Simpan</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        // Menggunakan nilai ID
        console.log('Nilai ID:', id);

        $.ajax({
            url: 'http://127.0.0.1:8000/api/produk/detail/' + id,
            method: 'GET',
            headers: {
                "Authorization": "Bearer " + token
            },
            success: function(response) {
                $('.ini-name').val(response.data.name);
                $('#mySelect').val(response.data.category_name);
                $('.ini-modal').val(response.data.modal);
                $('.ini-price').val(response.data.price);
                $('.ini-stock').val(response.data.stock);
                $('.ini-satuan').val(response.data.satuan);
                $('.ini-deskripsi').val(response.data.desc);
                $('.ini-status').val(response.data.is_active);
                $('.ini-is_jual').val(response.data.is_jual);
        ...
        // Mengirim permintaan pembaruan produk
        $(".update-produk").click(function() {
            // Ambil nilai input
            var productName = $(".ini-name").val();
            var category = $("#mySelect").val();
            var purchasePrice = $(".ini-modal").val();
            var sellingPrice = $(".ini-price").val();
            var description = $(".ini-deskripsi").val();
            var status = $(".ini-status").val();
            var isDisplayed = $(".ini-is_jual").val();
            var image = $(".ini-gambar").prop("files")[0];

            // Kirim permintaan pembaruan produk ke API
            $.ajax({
                url: "http://127.0.0.1:8000/api/produk/update-data/" + id,
                type: "PUT",
